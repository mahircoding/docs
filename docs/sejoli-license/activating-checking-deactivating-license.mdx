...
- [Lihat dokumentasi kami tentang cara mengintegrasikan auto update untuk plugin WordPress Anda.](docs/sejoli-license/implementasi-updater-untuk-plugin-wordpress.mdx)
- [Lihat dokumentasi kami tentang cara mengintegrasikan auto update untuk theme WordPress Anda.](docs/sejoli-license/implementasi-updater-untuk-theme-wordpress.mdx)
:::
...
Pembeli dapat mengaktifkan lisensi dari jarak jauh melalui sistem di plugin/theme/app Anda yang menggunakan API untuk memicu aktivasi.
...
Yang pertama adalah lisensi itu sendiri dan yang kedua adalah status lisensinya. Setelah kami mengaktifkan lisensi, kami akan mengubah status (di situs lokal kami) menjadi `active`. Hal ini agar kita dapat menampilkan tombol **Activate License** jika lisensinya belum diaktifkan, dan menyembunyikan tombol tersebut jika sudah diaktifkan. Lihat tangkapan layar di bawah ini:


![Changelog](./img/Xnapper-2024-10-21-02.57.12.png)
...
function sejoli_sample_activate_license() {
	...
	if( isset( $_POST['sejoli_license_activate'] ) ) {
		...
		$api_params = array(
			...
			'license'    => $license,
			'item_name'  => urlencode( SEJOLI_SAMPLE_ITEM_NAME ), // the name of our product in SEJOLI
			'url'        => home_url()
		);

		// Call the custom API.
		$response = wp_remote_post( SEJOLI_SAMPLE_STORE_URL, array( 'timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'item_name_mismatch' :
						...
						$message = sprintf( __( 'This appears to be an invalid license key for %s.' ), SEJOLI_SAMPLE_ITEM_NAME );
						break;
		...
		if ( ! empty( $message ) ) {
			$base_url = admin_url( 'plugins.php?page=' . SEJOLI_SAMPLE_PLUGIN_LICENSE_PAGE );
		...
		wp_redirect( admin_url( 'plugins.php?page=' . SEJOLI_SAMPLE_PLUGIN_LICENSE_PAGE ) );
		exit();
	}
}
...
![Changelog](./img/Xnapper-2024-10-21-03.17.38.png)
...
```bash
function sejoli_sample_theme_check_license() {
...
$api_params = array(
	...
	'item_name'  => 'Sample Plugin', // the name of our product in SEJOLI
	'url'        => home_url(),
);
...
```bash
{
  ...
  "payment_id": 32,
  "customer_name": "Slamet Baraka",
  "customer_email": "dev-slamet@gmail.com"
}
```
